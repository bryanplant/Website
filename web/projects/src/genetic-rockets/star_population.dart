...
class StarPopulation {
  ...
  StarPopulation(int maxStars) {
    stars = new Queue();
    newStarTimer = new Duration(milliseconds: 200);
    lastTime = new DateTime.now();
    possibleColors = [
      new StarColor(155, 176, 255),
      new StarColor(170, 191, 255),
      new StarColor(202, 215, 255),
      new StarColor(248, 247, 255),
      new StarColor(255, 244, 234),
      new StarColor(255, 210, 161),
      new StarColor(255, 204, 111)
    ];
  ...
  void newStar(bool fade, CanvasElement canvas) {
    ...
    do {
      ...
      for (Star star in stars) {
        if ((x - star.x).abs() < star.width &&
            (y - star.y).abs() < star.width) {
          valid = false;
          break;
        }
      }
    } while (!valid);
    StarColor color = possibleColors.elementAt(
        rand.nextInt(possibleColors.length));
    var r = color.r;
    var g = color.g;
    var b = color.b;
